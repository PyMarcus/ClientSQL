...
-- exibir placa e o nome dos donos de todos os veiculos
select c.nome, v.placa 
from cliente as c
join veiculo as v on v.cliente_cpf = c.cpf;

-- exibir o cpf e o nome do cliente que possui o veiculo de 
-- de placa JJJ-2020
select 
	c.cpf, c.nome
from
	cliente as c 
join veiculo as v on c.cpf = v.cliente_cpf and placa = 'JJJ-2020';

-- exiba a placa e a cor dos veiculos que estao
-- estacionados no patio 1
select v.placa, v.cor
from veiculo as v
join cliente as c on c.cpf = v.cliente_cpf; 

-- exiba endereço a data de entrada e saida dos estacionamentos
-- do veiculo de placa JEG-1010
select e.dtEntrada, e.dtSaida, p.ender
from estaciona as e join patio as p on e.patio_num = p.num where e.veiculo_placa = 'JEG-1010'; 

-- exiba a quantidade de veiculos de cor branca
-- que estacionaram
select count(*) from
veiculo as v
join estaciona as e on v.placa = e.veiculo_placa and v.cor = 'Branco';

-- Liste todos os clientes que possuem carro de modelo 1
select c.nome
from cliente as c
join veiculo as v on v.modelo_codMod = 1 and c.cpf = v.cliente_cpf;


-- Exiba nome clientes patio 2
select c.nome
from cliente as c
join veiculo as v
on c.cpf = v.cliente_cpf join estaciona
as e on e.patio_num = 2 and v.placa = e.veiculo_placa;
